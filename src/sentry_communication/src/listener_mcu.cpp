...
void serialDataCallback(const std_msgs::String::ConstPtr &msg) {
    ...
    auto sentryInfo = deserialize<navCommand_t>(data);
    if ((sentryInfo.head != 0x20) || (sentryInfo.tail != 0x21)){
        return;
    }
    printf("帧头帧尾匹配成功\n");
    ros::param::set("/real_world/x", sentryInfo.coo_x);
    ros::param::set("/real_world/y", sentryInfo.coo_y);
